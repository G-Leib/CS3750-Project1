...
public class Receiver {
    ...
    public static byte[] parseDecryptedMsg(String dsMsgFname, String msgOutFname) throws Exception {
        ...
        // Get first 128 bytes
        byte[] digSig = Arrays.copyOfRange(dsMsg, 0, dsSize);
    ...
    public static void rsaDecrypt(byte[] digSig) throws Exception {
        ...
        do {
            byte[] digSigFirst128 = Arrays.copyOfRange(digSig, 0, 128);
            digDigest = cipher.update(digSigFirst128);
            digSig = Arrays.copyOfRange(digSig, 128, digSig.length);
        } while (digSig.length > 128);

        digDigest = cipher.doFinal(digSig);
